...
MCU transport remains an alternative: the app automatically selects a single clearly named MCU/DAW port, and **Play button input (MCU)** can override it. If using that alternative, select the controller's MCU/HUI port and MCU mode, as described in [Arturia's transport setup](https://support.arturia.com/hc/en-us/articles/8905862388508-KeyLab-Essential-mk3-DAW-Integration). Manual port choices last until reload; automatic detection runs again on connection.

Supported messages are the KeyLab CC21 mapping above, MIDI Start/Continue, and MCU Play (note 94 on the selected transport port). Releases, held CC/MCU presses, clock, and Stop do not advance; duplicate transport signals within 150 ms are coalesced. Ordinary note 94 on a separate piano port is still evaluated as a note. Other custom CC mappings, HUI, and SysEx/MMC are not supported; no extra SysEx permission is requested. Stop diagnostic recording before testing phrase advancement: it intentionally pauses MIDI actions.
