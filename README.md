...
For the KeyLab Essential Mk3, keep the regular **MIDI** port selected for piano keys. The observed Play mapping sends **CC21 on channel 1** through this same port (`B0 15 7F` on press, `B0 15 00` on release). This mapping is recognized automatically when the selected piano port is named `KL Essential <size> mk3 MIDI` or `KeyLab Essential <size> mk3 MIDI`. No controller mode change or separate Play port is required for it. CC21 from unrelated devices or other ports is not treated as Play.

There is only one input selector for practice. On connection, the app prefers the recognized KeyLab regular MIDI port; an existing explicit selection is preserved while that port remains connected. Manual port choices last until reload. Other ports remain visible in the selector and diagnostics, but there is no separate MCU routing or Play-port setup.

Supported Play messages are the KeyLab CC21 mapping above and MIDI Start/Continue, all on the selected piano input. Releases, held CC presses, clock, and Stop do not advance; duplicate transport signals within 150 ms are coalesced. Note 94 is an ordinary piano note, never a Play command. Other custom CC mappings, MCU/HUI transport, and SysEx/MMC are not supported; no extra SysEx permission is requested. Stop diagnostic recording before testing phrase advancement: it intentionally pauses MIDI actions.
